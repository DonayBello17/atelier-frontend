...
const stats = useMemo(() => {
  ...
  const unidades = ventasActivas.reduce((acc, venta) => {
    return acc + (Number(venta.unidades) || 0);
  }, 0);

  const totalVentas = ventasActivas.length;
  const totalAnuladas = ventasAnuladas.length;
  ...
  const guardarVenta = async () => {
    ...
    try {
      ...
      if (res.data?.id_venta) {
        abrirFactura(res.data.id_venta);
      }
    } catch (err) {
  ...
  const abrirFactura = async (idVenta) => {
    try {
      ...
      if (!ventaFinal) {
        ...
        ventaFinal = (ventasRes.data.data || []).find((v) => String(v.id_venta) === String(idVenta));
      }

      setFactura({
        venta: ventaFinal || { id_venta: idVenta },
        items: res.data.data || [],
      });
    } catch (err) {
  ...
  const anularVenta = async (venta) => {
    ...
    const motivo = window.prompt(
      'Motivo de anulación:',
      'Venta anulada por corrección'
    );

    if (motivo === null) return;

    const confirmar = window.confirm(
      '¿Seguro que quieres anular esta venta? El stock será devuelto al inventario.'
    );
    ...
    try {
      ...
      await api.post(`/ventas/${venta.id_venta}/anular`, {
        motivo,
      });
  ...
  const imprimirFactura = () => {
    ...
    const filas = itemsFactura.map((item) => `
      <tr>
        <td>${item.producto}</td>
        <td>${item.talla}</td>
          ...
          <div class="invoice">
            <div class="top">
              <div>
                ...
                <div class="muted">Moda que trasciende</div>
                <div class="muted">Sistema de ventas e inventario</div>
              </div>

              <div class="invoice-number">
                <h1>Factura #${venta.id_venta}</h1>
                <div class="muted">Fecha: ${formatDate(venta.fecha)}</div>
                <div class="muted">Cliente: ${venta.cliente || 'Sin cliente'}</div>
                <div class="muted">Teléfono: ${venta.telefono || 'N/A'}</div>
                <div class="muted">Email: ${venta.email || 'N/A'}</div>
              </div>
            </div>
            ...
            <div class="total">
              <div class="total-box">
                ...
                <div class="total-value">${formatMoney(venta.total)}</div>
              </div>
            </div>

            <div class="thanks">
              Gracias por su compra. Esta factura fue generada por ATELIER.
            </div>
          </div>
    ...
    : 'Registra ventas, descuenta inventario y genera facturas imprimibles para tus clientes.'}
          ...
          <section className="stats-grid">
            <div className="stat-card">
              <div className="stat-label">Ingresos activos</div>
            ...
            {puedeVender && mostrarForm && (
              <div className="glass-card">
                ...
                <p className="card-subtitle">
                  Selecciona un cliente y agrega los productos vendidos.
                </p>
                ...
                <div className="sale-total-box">
                  <div>
                    ...
                    <div className="card-subtitle" style={{ margin: 0 }}>
                      Al guardar, el stock se descuenta automáticamente.
                    </div>
                  </div>
  ...
  {esCliente
    ...
    : 'Busca por cliente o número de venta.'}
              ...
              <div className="search-meta">
                Mostrando {ventasFiltradas.length} de {ventasBase.length} ventas.
              </div>
                    ...
                    {ventasFiltradas.map((venta) => (
                      <tr key={venta.id_venta}>
                        ...
                        <td>
                          <strong
  style={{
    textDecoration: venta.estado === 'anulada' ? 'line-through' : 'none',
    opacity: venta.estado === 'anulada' ? 0.45 : 1,
  }}
>
    ...
    {venta.estado === 'anulada' ? 'Anulada' : 'Activa'}
                        ...
                        <td>
                          <div className="actions-row" style={{ marginTop: 0 }}>
                            ...
                            <button
                              className="btn-gold"
                              onClick={() => abrirFactura(venta.id_venta)}
                            >
                              Factura
                            </button>
    ...
    Anular
          ...
          {factura && (
            <div className="modal-backdrop">
              <div className="modal-card">
                <div className="modal-header">
                  <h2 className="modal-title">Factura #{factura.venta.id_venta}</h2>

                  <div className="actions-row" style={{ marginTop: 0 }}>
                    <button className="btn-gold" onClick={imprimirFactura}>
                      Imprimir factura
                    </button>

                    <button className="btn-dark" onClick={() => setFactura(null)}>
                      Cerrar
                    </button>
                  </div>
                </div>
                ...
                <div className="invoice-preview">
                  <div className="invoice-row">
                    <div>
                      <div className="invoice-brand">ATELIER</div>
                      <div className="invoice-muted">Moda que trasciende</div>
                      <div className="invoice-muted">Sistema de ventas e inventario</div>
                    </div>

                    <div style={{ textAlign: 'right' }}>
                      <h2 style={{ margin: '0 0 8px' }}>Factura #{factura.venta.id_venta}</h2>
                      <div className="invoice-muted">Fecha: {formatDate(factura.venta.fecha)}</div>
                      <div className="invoice-muted">Cliente: {factura.venta.cliente || 'Sin cliente'}</div>
                      <div className="invoice-muted">Teléfono: {factura.venta.telefono || 'N/A'}</div>
                      <div className="invoice-muted">Email: {factura.venta.email || 'N/A'}</div>
                    </div>
                  </div>

                  {factura.items.map((item, index) => (
                    <div key={index} style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 12, padding: '12px 0', borderBottom: '1px solid #eee' }}>
                      <div>
                        <strong>{item.producto}</strong>
                        <div className="invoice-muted">
                          Talla {item.talla} · {item.color || 'Sin color'} · Cantidad {item.cantidad} · {formatMoney(item.precio_unitario)}
                        </div>
                      </div>
                  ...
                  <div className="invoice-total">
                    Total: {formatMoney(factura.venta.total)}
                  </div>
                </div>
              </div>
            </div>
